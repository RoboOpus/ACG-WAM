...
for number,title,description,score in real_tasks:
    ...
    real_cards.append(f'''<article class="video-card" data-real-card>
      ...
      <h3>{title}</h3><p>{description}</p><p class="task-score">{score}</p>
...
<meta name="theme-color" content="#f7f8fc"><meta property="og:title" content="ACG-WAM: World-Action Modeling">
...
<div class="eyebrow">Geometric prediction for robot learning</div>
<div class="hero-title"><h1 id="paper-title"><span class="acronym">ACG-WAM</span>World-Action Modeling<span class="subtitle">via Action-Conditioned<br> Geometric Latent Prediction</span></h1>
<p class="lede">What geometry follows an action?<br><span>Learn from future geometric targets. Act from current observations.</span></p></div>
...
<div class="hero-layout"><div class="geometry-card"><div class="eyebrow">The ACG-JEPA objective</div><h2>Connect actions<br> to geometric change.</h2><div class="prediction-path" aria-label="Current observation, intervening actions and a horizon predict future geometric features"><div class="prediction-inputs"><span>Current observation <b>I<sub>t</sub></b></span><span>Intervening actions <b>a<sub>t:t+k</sub></b></span><span>Prediction horizon <b>k</b></span></div><span class="prediction-arrow" aria-hidden="true">↓</span><div class="prediction-target">Predict future geometric features</div></div><p>Supervised by a frozen VGGT teacher during training.</p><div class="geometry-footer"><span class="geometry-dot" aria-hidden="true"></span>Teacher and predictor removed at inference</div></div>
<div class="hero-media">{video(hero['src'],hero['poster'],'Bimanual fruit placement with ACG-WAM',True)}<div class="media-caption"><span>From representation to manipulation</span><span>TRON2 × WUJI · 1× speed</span></div></div></div>
...
<div class="video-grid real-grid">{''.join(real_cards)}</div><p class="note">These are selected successful demonstrations. The success rates above come from the complete evaluation protocol, including unsuccessful trials. Videos are muted; use the player controls to play or enter fullscreen.</p>
...
<footer><div class="shell"><p><strong>ACG-WAM</strong><br>Action-conditioned geometric latent prediction</p><p>Research project · <a href="https://github.com/RoboOpus/ACG-WAM">Code &amp; resources</a><br><a href="#top">Back to top ↑</a></p></div></footer>
</body></html>'''
